...
from adws.adw_modules.utils import setup_logger, parse_json, check_env_vars
...
from adws.adw_modules.r2_uploader import R2Uploader
...
def resolve_blocker_issues(
    ...
    Args:
        ...
        issue: Issue object (platform-agnostic)
        provider: Issue provider for posting comments
        adw_id: ADW workflow ID
        worktree_path: Path to the worktree
        logger: Logger instance
    """
    ...
    for i, blocker_issue in enumerate(blocker_issues, 1):
        logger.info(f"Resolving blocker {i}/{len(blocker_issues)}: {blocker_issue.issue_description}")

        # Create patch plan
        plan_response = create_review_patch_plan(blocker_issue, i, adw_id, logger, working_dir=worktree_path)
...
def _is_image_url(url: str) -> bool:
    ...
    Returns:
        True if the URL ends with an image extension
    """
    image_extensions = ('.png', '.jpg', '.jpeg', '.gif', '.webp', '.bmp', '.svg')
    return url.lower().endswith(image_extensions)


def _render_screenshot_markdown(url: str, label: str) -> str:
    """Render a screenshot URL as appropriate markdown.

    If the URL points to an image, render as inline image.
    Otherwise, render as a link to the file.
    ...
    Returns:
        Markdown string for the screenshot
    """
    if _is_image_url(url):
        return f"![{label}]({url})"
    else:
        # Not an image - render as a link instead
        return f"[{label} (text file)]({url})"
...
def build_review_summary(review_result: ReviewResult) -> str:
        ...
        if blockers:
            ...
            for issue in blockers:
                ...
                if issue.screenshot_url and issue.screenshot_url.startswith("http"):
                    summary_parts.append(f"  - {_render_screenshot_markdown(issue.screenshot_url, 'Issue Screenshot')}")
    ...
    # Add screenshots section - only include actual images
    if review_result.screenshot_urls:
        # Filter to only actual image URLs
        image_urls = [url for url in review_result.screenshot_urls if url.startswith("http") and _is_image_url(url)]
        text_urls = [url for url in review_result.screenshot_urls if url.startswith("http") and not _is_image_url(url)]

        if image_urls:
            summary_parts.append(f"\n## 📸 Screenshots")
            summary_parts.append(f"Captured {len(image_urls)} screenshots\n")

            # Use uploaded URLs to display as inline images
            for i, screenshot_url in enumerate(image_urls):
                summary_parts.append(f"### Screenshot {i+1}")
                summary_parts.append(f"![Screenshot {i+1}]({screenshot_url})\n")

        if text_urls:
            summary_parts.append(f"\n## 📄 Review Artifacts")
            summary_parts.append(f"{len(text_urls)} text-based review artifacts\n")
            for i, artifact_url in enumerate(text_urls):
                summary_parts.append(f"- [Review Artifact {i+1}]({artifact_url})")

        # Handle local paths (fallback)
        local_paths = [url for url in review_result.screenshot_urls if not url.startswith("http")]
        if local_paths:
            summary_parts.append(f"\n## 📁 Local Files")
...
def main():
    ...
    # Resolve issue using platform-agnostic provider
    try:
        issue: Issue = resolve_issue(issue_number)
        temp_logger.info(f"Resolved issue from {issue.source.value}: {issue.title}")
    except Exception as e:
        temp_logger.error(f"Error resolving issue: {e}")
        sys.exit(1)

    # Get the provider for this issue (for comments/updates)
    provider: IssueProvider = get_provider_for_issue(issue)

    # Notify about starting review
    provider.add_comment(
        issue,
    ...
    # Check if already reviewed at this commit (skip duplicate reviews)
    current_commit = get_commit_hash(worktree_path)
    last_reviewed = state.get("last_reviewed_commit_sha")

    if current_commit and last_reviewed and current_commit == last_reviewed:
        skip_msg = f"Branch already reviewed at commit {current_commit[:8]}. Skipping duplicate review."
        logger.info(skip_msg)
        provider.add_comment(
            issue,
            format_issue_message(adw_id, "ops", f"⏭️ {skip_msg}"),
            adw_id,
        )
        sys.exit(0)
    ...
    # Enable Playwright MCP for browser automation during review (if template exists)
    mcp_enabled = enable_playwright_mcp(worktree_path, logger)
    if mcp_enabled:
        logger.info("Playwright MCP enabled for review")
    ...
    try:
        while review_attempt < MAX_REVIEW_RETRY_ATTEMPTS:
            ...
            resolve_blocker_issues(blocker_issues, issue, provider, adw_id, worktree_path, logger)
        ...
        # Always disable Playwright MCP after review (cleanup)
        if mcp_enabled:
            disable_playwright_mcp(worktree_path, logger)
            logger.info("Playwright MCP disabled after review")
    ...
    # Create commit message (issue already resolved earlier in main())
    logger.info("Creating review commit")
    commit_msg, error = create_commit(AGENT_REVIEWER, issue.to_minimal_dict(), issue_command, adw_id, logger, worktree_path)
    ...
    # Finalize git operations (push and PR/MR)
    # Note: This will work from the worktree context
    finalize_git_operations(state, logger, cwd=worktree_path)

    # Handle GitLab MR if needed (finalize_git_operations handles GitHub PRs)
    if issue.source == IssueSource.GITLAB:
        # Get current branch from state
        branch_name = state.get("branch_name")
        if branch_name:
            logger.info("Checking for GitLab merge request")
            ...
            if existing_mr:
                ...
                provider.add_comment(issue, f"{adw_id}_ops: ✅ Merge request updated: {existing_mr.url}", adw_id)
            else:
                # Create new MR
                logger.info("Creating new merge request")
                mr_title = f"[{adw_id}] {issue.title}"
                mr_body = f"Review for issue #{issue.id}\n\n{AGENT_REVIEWER}: Review completed in isolated worktree."
    ...
    provider.add_comment(
        issue,
        format_issue_message(adw_id, "ops", "✅ Isolated review phase completed"),
        adw_id,
    )

    # Save the commit SHA we reviewed (enables skip-if-already-reviewed on re-run)
    state.update(last_reviewed_commit_sha=current_commit)
