...
This module provides the core execution layer for Jerry's ADW system.
"""
...
# Import types from centralized data_types module
...
def extract_token_usage(result_message: Dict[str, Any]) -> Tuple[Optional[TokenUsage], Optional[List[ModelUsage]]]:
    """Extract token usage statistics from a Claude Code result message.

    Args:
        result_message: The parsed result message from Claude Code output

    Returns:
        Tuple of (TokenUsage, List[ModelUsage]) or (None, None) if extraction fails
    """
    try:
        # Extract aggregate token usage
        token_usage = TokenUsage.from_result_message(result_message)

        # Extract per-model usage breakdown
        model_usage_list = None
        model_usage_data = result_message.get("modelUsage", {})
        if model_usage_data:
            model_usage_list = [
                ModelUsage.from_model_entry(model_id, data)
                for model_id, data in model_usage_data.items()
            ]

        return token_usage, model_usage_list
    except Exception:
        return None, None
...
def get_claude_env() -> Dict[str, str]:
    ...
    """
    # Use the centralized function from utils
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if result_message:
                ...
                token_usage, model_usage = extract_token_usage(result_message)
