...
## Data Flow: How Workflow Prompts Execute

```mermaid
flowchart TD
    A[User runs /analyze file.py] --> B[Claude Code loads prompt]
    B --> C[Variables substituted]
    C --> D{Step 1: Read file}
    D --> E{Step 2: Analyze context}
    E --> F{Step 3: Find improvements}
    F --> G{Step 4: Create plan}
    G --> H{Step 5: Write output}
    H --> I[Plan saved to specs/]

    D -->|File not found| J[Early return: error]
    F -->|No issues| K[Early return: file looks good]
...
Each step in the workflow maps directly to agent actions. The agent creates a to-do list matching your workflow and executes sequentially.
...
### Template: Refactoring Workflow

```markdown
---
description: Refactor code following specific pattern
allowed-tools: Read, Edit, Bash
argument-hint: <file_path> <pattern>
---

# Refactoring Agent

## Purpose
Apply {{pattern}} refactoring to {{target_file}} safely.

## Variables
- **target_file**: $1
- **pattern**: $2 (extract-method|rename|simplify|split)

## Workflow

1. **Create backup state**
   - Run `git stash` if uncommitted changes exist
   - Note current git hash for rollback

2. **Analyze current structure**
   - Read {{target_file}} completely
   - Identify the code section matching {{pattern}} criteria
...
4. **Execute refactoring**
   ...
   - After each change, verify syntax is valid

5. **Verify correctness**
   - Run existing tests: `npm test` or `pytest`
   - If tests fail, rollback and report what broke
...
**Changes Made**: [list]
**Tests**: PASS/FAIL
**Rollback Command**: git checkout {{hash}} -- {{target_file}}
```
```

### Template: Test Generation Workflow

```markdown
---
description: Generate tests for a function or file
...
## Purpose
Create comprehensive tests for {{target_file}}.
...
1. **Analyze the target**
   ...
   - Identify input types and return types
