...
- Base your `Plan Format: Validation` on the validation steps from `spec_path` if provided
  ...
  - If not provided, determine appropriate validation based on the type of change
...
Focus on the following files based on your task:
...
Ignore files that are not relevant to your patch.
