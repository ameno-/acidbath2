"""State management for Jerry ADW composable architecture.
...
class ADWState:
    ...
    def update(self, **kwargs):
        ...
        Accepts core ADW fields plus 'metadata' for extensibility.
        """
        # Core fields that are always accepted
        core_fields = {
            "adw_id", "issue_number", "issue_source", "issue_ref",
            "branch_name", "plan_file", "issue_class", "worktree_path",
            "backend_port", "frontend_port", "model_set", "all_adws", "metadata"
        }
        ...
        for key, value in kwargs.items():
            if key in core_fields:
                ...
                # Store unknown fields in metadata for extensibility
    ...
    def get(self, key: str, default=None):
        ...
        Checks core fields first, then metadata.
        """
        if key in self.data:
            return self.data.get(key, default)
        # Check metadata for extended fields
    ...
    def update_issue(self, issue: "Issue") -> None:
        ...
        Stores minimal issue data plus source tracking for multi-source
        issue support (GitHub, Local, Prompt, Linear).
        ...
        self.data["issue_number"] = issue.id
        self.data["issue_source"] = issue.source.value
        self.data["issue_ref"] = issue.source_ref

        # Store minimal issue data in metadata for quick access
        if "metadata" not in self.data:
            self.data["metadata"] = {}
        self.data["metadata"]["issue"] = issue.to_minimal_dict()

    def get_issue_ref(self) -> Optional[str]:
        """Get the issue reference for re-fetching.

        Returns:
            Issue reference string (e.g., "github:123", "local:abc") or None.
        """
        return self.data.get("issue_ref")
    ...
    def save(self, workflow_step: Optional[str] = None) -> None:
        ...
        state_data = ADWStateData(
            ...
            issue_source=self.data.get("issue_source"),
            issue_ref=self.data.get("issue_ref"),
    ...
    def load(
        ...
        try:
            ...
            if logger:
                ...
                logger.debug(f"State: {json.dumps(state_data.model_dump(), indent=2)}")
    ...
    def to_stdout(self):
        ...
        # Output all fields including metadata
        output_data = {
            "adw_id": self.data.get("adw_id"),
            "issue_number": self.data.get("issue_number"),
            "issue_source": self.data.get("issue_source"),
