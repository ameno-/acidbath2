...
def cleanup_worktree_config_files(project_root: str, logger: logging.Logger) -> None:
    """Remove worktree-specific config files from main repository.

    These files contain hardcoded absolute paths to specific worktrees and should
    only exist within worktree directories, not in the main repository. This function
    prevents main repo pollution by cleaning up stale config files before creating
    new worktrees.

    Args:
        project_root: Path to the main project repository root
    ...
    logger.info("Cleaning up stale worktree config files from main repo")
    ...
    for filename in config_files:
        ...
        if os.path.exists(file_path):
            try:
                os.remove(file_path)
                logger.info(f"Removed stale config file: {filename}")
            except Exception as e:
                logger.warning(f"Failed to remove {filename}: {e}")
        else:
            logger.debug(f"Config file {filename} not found (already clean)")
